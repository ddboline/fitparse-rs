...
impl TimestampField {
    ...
    /// # Panics if timezone conversion is ambiguous
    fn to_date_time(self) -> OffsetDateTime {
        ...
        match self {
            Self::Local(value) => {
                ...
                ref_date.assume_timezone(local).unwrap() + Duration::seconds(value)
            }
            Self::Utc(value) => ref_date.assume_utc() + Duration::seconds(value),
        }
    }
}
